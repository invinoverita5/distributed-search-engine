...
public class KVSClient implements KVS {
  ...
  synchronized void downloadWorkers() throws IOException {
    System.err.println("[KVSClient] downloadWorkers: requesting http://" + coordinator + "/workers");
    String result = new String(HTTP.doRequest("GET", "http://"+coordinator+"/workers", null).body());
    System.err.println("[KVSClient] downloadWorkers: got response, parsing...");
  ...
  public void putRow(String tableName, Row row) throws FileNotFoundException, IOException {
    ...
    String target = "http://"+workers.elementAt(workerIndexForKey(row.key())).address+"/data/"+tableName;
    ...
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      try {
        byte[] response = HTTP.doRequest("PUT", target, row.toByteArray()).body();
        String result = new String(response);
        if (!result.equals("OK")) {
  ...
  public void putRowBatch(String tableName, List<Row> rows, boolean merge) throws IOException {
    if (!haveWorkers)
      downloadWorkers();

    // Group rows by worker
    Map<Integer, List<Row>> rowsByWorker = new HashMap<>();
    for (Row row : rows) {
      int workerIndex = workerIndexForKey(row.key());
      if (!rowsByWorker.containsKey(workerIndex)) {
        rowsByWorker.put(workerIndex, new ArrayList<Row>());
      }
      rowsByWorker.get(workerIndex).add(row);
    }

    // Send batches to each worker
    for (Map.Entry<Integer, List<Row>> entry : rowsByWorker.entrySet()) {
      int workerIndex = entry.getKey();
      List<Row> workerRows = entry.getValue();

      String target = "http://"+workers.elementAt(workerIndex).address+"/batch/"+tableName;
      if (merge) {
        target += "?merge=true";
      }
      ...
      for (int attempt = 0; attempt < maxRetries; attempt++) {
        try {
          // Serialize all rows for this worker
          ByteArrayOutputStream baos = new ByteArrayOutputStream();
          for (Row row : workerRows) {
            baos.write(row.toByteArray());
          }

          byte[] response = HTTP.doRequest("PUT", target, baos.toByteArray()).body();
          String result = new String(response);
          if (!result.equals("OK")) {
            throw new RuntimeException("PUT returned something other than OK: "+result);
          }
